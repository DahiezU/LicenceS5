...
League of legends semble etre un jeu d'équipe de stratégie. Le nombre giganteste de héros et de classes peut peut-être déstabiliser les nouveaux joueurs et
il est peut-être statégiquement intérressant de connaitre les meilleurs héros , meilleurs combinaisons de héros, les meilleurs emplacements ,etc...  

Nous allons nous intéresser à la classe du personnage, ses caractéristiques et les différents emplacements sur les maps . 
Nous allons donc à l'aide des données de player_team_lol_data pouvoir determiner l'intérêt des différentes classes et leurs spécificitées ainsi que les différents placements .  
...
Hypothèse: Quel est la problalité de gagner en jouant dans une équipe avec le héros le plus joué en général.
...
On remarque que les pourcentages sont assez peu éloignés. 

Nous allons maintenant chercher à savoir si Lucian est plus joué parcequ'il a plus d'atouts pour faire des kills. 


On utilise la loi Binomiale pour calcuer la probalité de gagner en ayant ce hero dans l'équipe. 
...
La probabilité de gagner avec ce héro dans l'équipe est seulement de 0.44, cela nous montre donc que ce héro est très populaire mais il n'est pas forcément très bien joué par les joueurs de lol ou très fort. Plusieurs paramètres  peuvent impacter cette statistique mais cela reste intéréssant. 



Pour comparer avec les reste des héros on va chercher le héro le plus joué lors de victoire. 
```{r}
...
Caitlyn semble être le héro le plus joué lors de victoires , il fait lui aussi parti des  héros les plus populaires.
...
Une équipe contenant  Caitlyn a une probabilité de 0.54 de gagner . Cette probabilité est bien supérieure à celle de Lucian. 
...
Donc il est stratégiquement plus intéligent de bien observé les kills de la jungle. 
...
On peut admètre l’hypothèse de normalité sur la loi de (X, Y).
...
Observe que généralement la moyenne des kills toutes classes confondu 
lors d'une partie gagnante se trouve aux alentours de 5,49  et le troisième quartile se trouve aux alentours de 7 . 

On va observer si jouer une classe  a une influence sur le nombre kills . 
On tire 30 résultats de chaques nombres de kills pour la classe Fighter : 
...
Nous avons une liste de 30  valeurs de kills efféctué par des membres de la classe fighter lors de victoire .
...
On suppose que le nombre de kills d’une classe peut être modélisée par une var X ∼
...
Dans l’énoncé, on observe :  
  • la valeur de BOTTOM
) pour chacun des n individus d’un échantillon avec n = 261, µ
inconnu et σ =  4.031703,  
  • la valeur de TOP 
) pour chacun des n individus d’un échantillon avec n = 186, µ
inconnu et σ = 4.445639  
  • la valeur de MIDDLE 
) pour chacun des n individus d’un échantillon avec n = 167, µ
inconnu et σ = 4.445639  
  • la valeur de JUNGLE 
) pour chacun des n individus d’un échantillon avec n = 153, µ
...
. 
Ces tests nous permettent d'assurer l'homogénéité des parties entre les lanes ;
TOP - JUNGLE,
TOP - MIDDLE,
MIDDLE - JUNGLE
...
Nous pouvons donc en conclure que les parties dans la lane BOTTOM sont jouées différement par le joueur 1.
...
Dans cette première visualisation nous pouvons constater que le héro le plus joué lors de victoire est Caitlyn.


\newpage
### Composition lors de Victoire :
Regardons mainenant si  stratégiquement il est plus malin de placer certaines classes à cetains endroits. 
...
```
En middle on observe un graphique plutôt equilibré. Avec une légére absence de mage dans cette lane. La lane middle semble être praticable par toutes les classes de héros.
...
En bottom on observe un graphique contenant majoritairement des Supports(32 %) et des Marksman (36%). Cette lane doit donc subir de nombreux dégats pour que les Supports soient aussi présents, ou l'alliance Marskmam et Support est peut-être très efficasse. 
...
Par exemple nous avons remarqué que le fait de jouer un Support limit gégéralement le nombre de kills mais augmente le nombre d'assits. Alors que jouer Fighter permet de faire  plus de kills que la moyenne .    
On a aussi remarqué que tout les personages ,même au sein d'une même classe , ne sont pas égaux .
Caitlyn reste une héros statistiquement plus fort que les autres. 
On a également pus observer que la lane Bottom est une lane de forte activité, beaucoup de kills y sont fait .   
...
Nous avons bien partagé le projet avec Ulysse , nous avons fait la moitié des Phénomènes  et la moitié des tests d'hypothèses chacuns. J'ai plus travaillé sur la partie concerant les classes et les héros .
...
J'ai vraiment aimé travailler sur ce projet car nous avons reussi à bien nous investir dans la compréhension du gamplay de ce jeu et de ses actuces. Nous avons reussi  nos calculs basés sur les aspects les plus attirants du jeu ( les personnages et les deplacements) et j'ai vraiment aimé y participer.
